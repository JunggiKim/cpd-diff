    ...
    const fingerprinted = await Promise.all(
      relevant.map(async (group) => {
        ...
        if (primary === undefined)
          throw new Error("Clone group has no occurrences");
        return {
          fingerprint: fingerprintFragment(
            await fragment(repositoryRoot, primary),
          ),
          group,
        };
      }),
...
async function fragment(
  repositoryRoot: string,
  occurrence: { file: string; startLine: number; endLine: number },
) {
  const source = await readFile(
    path.join(repositoryRoot, occurrence.file),
    "utf8",
  );
  ...
  if (
    ...
    occurrence.endLine > lines.length
  ) {
    throw new Error(
      `Clone occurrence is outside the source file: ${occurrence.file}`,
    );
  }
...
function effectiveExtensions(options: CliOptions): readonly string[] {
  const extensions =
    options.extension.length > 0
      ? options.extension
      : LANGUAGE_EXTENSIONS[options.language];
  if (extensions === undefined)
    throw new Error(`No default extensions for language: ${options.language}`);
