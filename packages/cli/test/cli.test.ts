...
const fakeEngine = new URL("../../engines/test/fixtures/fake-engine.mjs", import.meta.url).pathname;
...
describe("runCli", () => {
  ...
  beforeEach(async () => {
    repository = path.join(process.cwd(), "temp-doc", "etc", "cli-test", randomUUID());
    await mkdir(path.join(repository, "src"), { recursive: true });
    git(repository, "init", "-q", "-b", "main");
    git(repository, "config", "user.name", "Test");
    git(repository, "config", "user.email", "test@example.invalid");
    await writeFile(path.join(repository, "src", "base.ts"), duplicateSource("base"));
    git(repository, "add", ".");
    git(repository, "commit", "-qm", "base");
    await writeFile(path.join(repository, "src", "changed.ts"), duplicateSource("changed"));
    git(repository, "add", ".");
  ...
  test("returns 0 for warn-only and emits stable JSON", async () => {
    const exitCode = await runCli([...baseArguments(), "--warn-only", "--format", "json"], io());
    const report = JSON.parse(stdout) as { schemaVersion: string; summary: { violationCount: number } };
    expect(exitCode).toBe(0);
    expect(report).toMatchObject({ schemaVersion: "1.0.0", summary: { violationCount: 1 } });
    expect(stderr).toBe("");
  });

  test("returns 0 when no selected files can form a clone", async () => {
    const exitCode = await runCli([...baseArguments(), "--include", "docs/**"], io());
    expect(exitCode).toBe(0);
  ...
  function baseArguments(): string[] {
    return [
      "--base", "HEAD~1", "--head", "HEAD", "--engine", "jscpd",
      "--engine-path", fakeEngine, "--language", "typescript",
      "--minimum-tokens", "10", "--minimum-lines", "2",
      "--mode", "changed-lines",
    ];
  }
