...
describe("filterChangedFileGroups", () => {
  test.each([
    ...
    [
      "C,B,B",
      [occurrence("c1.ts", 1), occurrence("b1.ts", 1), occurrence("b2.ts", 1)],
      true,
    ],
    [
      "changed self clone",
      [occurrence("c1.ts", 1), occurrence("c1.ts", 20)],
      true,
    ],
    [
      "baseline self clone",
      [occurrence("b1.ts", 1), occurrence("b1.ts", 20)],
      false,
    ],
  ])(
    "applies the pair truth table for %s",
    (_description, occurrences, expected) => {
      const group = createCloneGroup({ lines: 5, occurrences, tokens: 20 });

      expect(
        filterChangedFileGroups([group], new Set(["c1.ts", "c2.ts"])),
  ...
  test("rejects unsafe changed paths", () => {
    expect(() =>
      filterChangedFileGroups([], new Set(["../escape.ts"])),
